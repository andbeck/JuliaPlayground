...
for i in 1:nrep
     ...
     tstop = Int.(60*60*24*364.25*3000) #simulation time = 3000 years
     Δt = Int.(60*60*24*364.25) #sample every year
...
df = DataFrame()
for i in 1:nrep
     pers_adbm = species_persistence.(adbm_res[i], last = 1)
     ...
     noinstab_adbm = [x[:t][end] == 94413600000 for x in adbm_res[i]]
     noinstab_niche = [x[:t][end] == 94413600000 for x in niche_res[i]]
...
@df df groupedboxplot(:foodwebmodel, :pers, group = :rewiringmodel
     , legend = :topleft, legendtitlefontsize = 7, legendfontsize = 7, legendtitle = "rewiring model"
     , xlabel = "food web model", ylabel = "persistence")
savefig("extinctions/persistence_matched.png")