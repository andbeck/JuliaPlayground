...
import BioEnergeticFoodWebs.nichemodel
using LinearAlgebra
using Distributions
using Plots
...
function random_adbm_parameters(S)
    ...
    c_i = rand(S)
    c_cst = 1
    return (S = S, M = sort(M), e = rand(e_cst), a = 10^(rand(ar_cst)), ai = rand(ar_expi), aj = rand(ar_expj), Ea = rand(Ea), b = rand(ht_threshold), h = rand(ht_cst), hi = rand(ht_expi), hj = rand(ht_expj), Eh = rand(Eh), n = n_cst, ni = rand(n_expi), ci = c_i, c_cst = c_cst)
end

function informed_adbm_parameters(S)
    #mass
    μM = Uniform(1,15) 
    σM = Uniform(1,10)
    mM = rand(μM)
    ...
    #feeding rates parameters

    μ = -16.82
    q1 = -21.29
    q3 = -11.69
    σ1 = (μ - q1) / 0.675 #estimate std from quartile 1
    σ2 = -(μ - q3) / 0.675 #estimate std from quartile 2
    σ = (σ1 + σ2)/2 #avg between the two
    tmin = μ - (2*σ)
    tmax = μ + (2*σ)
    ar_cst = TruncatedNormal(μ, σ, tmin, tmax)
    ...
    tmin = μ - (2*σ)
    tmax = μ + (2*σ)
    ar_expj =  TruncatedNormal(μ, σ, tmin, tmax) #consumer
    ...
    tmin = μ - (2*σ)
    tmax = μ + (2*σ)
    ar_expi = TruncatedNormal(μ, σ, tmin, tmax) #resource
    ...
    tmin = μ - (2*σ)
    tmax = μ + (2*σ)
    Ea = TruncatedNormal(μ, σ, tmin, tmax)
    ...
    tmin = μ - (2*σ)
    tmax = μ + (2*σ)
    ht_cst = TruncatedNormal(μ, σ, tmin, tmax)

    ht_threshold = Uniform(1e-4,1) #modif

    ht_expi = -0.45 #resource (from Binzer)
    ht_expj = 0.47 #consumer (from Binzer)
    ...
    σ2 = -(μ - q3) / 0.675 #estimate std from quartile 2
    σ = (σ1 + σ2)/2 #avg between the two
    # truncate with quartiles +/- 0.1
    tmin = μ - (2*σ)
    tmax = μ + (2*σ)
    Eh = TruncatedNormal(μ, σ, tmin, tmax)
    ...
    return (S = S, M = sort(M), a = exp(rand(ar_cst)), ai = rand(ar_expi), aj = rand(ar_expj), Ea = rand(Ea), b = rand(ht_threshold), h = exp(rand(ht_cst)), hi = ht_expi, hj = ht_expj, Eh = rand(Eh), n = n_cst, ni = rand(n_expi), e = rand(e_cst), ei = e_expi, ci = c_i, c_cst = c_cst)
end
...
function empirical_adbm_parameters(S)
    ...
    return (S = S, M = sort(M), e = e_cst, a = 10^(ar_cst), ai = ar_expi, aj = ar_expj, Ea = Ea, b = rand(ht_threshold), h = ht_cst, hi = ht_expi, hj = ht_expj, Eh = Eh, n = n_cst, ni = rand(n_expi), ci = c_i, c_cst = c_cst)
...
"""
function generateADBM(p::NamedTuple; use_c::Bool = false)
    ...
    S = p.S
    H = zeros(Float64,(S,S))
    c_vec = use_c ? p.c_cst .* p.ci : zeros(S)
    for i = 1:S , j = 1:S
      if ratios[j,i] < p.b
        H[j,i] =  (p.h / (p.b - ratios[j,i])) + c_vec[i]
      else
        H[j,i] = Inf
      end
    end
...
function profitability(p::NamedTuple; use_c::Bool = false)
        #get feeding rates 
        E = p.e .* p.M
        A = p.a .* (p.M' .^ p.ai) .* (p.M .^ p.aj) # a * prey * pred
        ratios = (p.M ./ p.M')' #PREDS IN ROWS : PREY IN COLS
...
"""
function ADBM_foodweb(S ; parameters = :random, use_c::Bool = false)
    co = 1.0
    np = 0
    m = 1e7
    #=
    ...
    elseif parameters == :informed
        adbm_p = informed_adbm_parameters(S)
    else
        println("parameters must be either :random, :informed or :empirical")
    end
    while (co >= 0.5) | (np < 1) | (m > 200e6)
        ...
        A = generateADBM(adbm_p, use_c = use_c)
        #check connectance
        co = sum(A / (S^2))
...
function ADBM_niche_equiv(S; parameters = :random, C = 0.15)
    global A = NaN
    while A == NaN
        An = nichemodel(S, C)
        co_n = sum(An) / S^2
        ...
        while (diff_co | diff_np | diff_height) & jcount
            global j = j+1
            global A, adbm_p = ADBM_foodweb(S, parameters = parameters)
            co = sum(A) / S^2
            np = sum(sum(A, dims = 2) .== 0)
            tl = trophic_rank(A)
            H = maximum(tl)
